...
class DistanceDataSource(
    ...
    suspend fun distance(srcLon: Double, srcLat: Double, destLon: Double, destLat: Double): Result<Int> = dispatchedRequest {
        distanceApi.getDistance(srcLon, srcLat, destLon, destLat).getOrThrow().distance.toInt()
    }
}